...
async def main():
    ...
    if not os.path.exists(file_to_save):
        ...
        start = dt.date(year=2013, month=4, day=28)
        df = pd.DataFrame(columns=['Date', 'Price', 'Volume', 'Marketcap'])
        while start != dt.date.today():
            histories = await coinpaprika.get_coin_history(coin_id='btc-bitcoin', start=start, interval='1h')
            start = dt.datetime.strptime(histories[-1].timestamp, '%Y-%m-%dT%H:%M:%SZ').date() + dt.timedelta(days=1)
            for history in histories:
                date = dt.datetime.strptime(history.timestamp, '%Y-%m-%dT%H:%M:%SZ')
                data_row = [date.date(), history.price, history.volume_24h, history.market_cap]
                df.loc[-1, :] = data_row
                df.index = df.index + 1
        ...
        print(f'Loaded {df.size} samples')
    ...
    num_nodes = [200, 200, 150]  # Number of hiddn nodes in each layer of the deep LSTM stack we're using
    ...
    w = tf.get_variable('w', shape=[num_nodes[-1], 1], initializer=xavier_initializer())
...
def visualize_predictions(all_mid_data, df, predictions_over_time, x_axis_seq, ep):
    best_prediction_epoch = ep  # replace this with the epoch that you got the best results when running the plotting code
